...
use std::sync::RwLock;
#[derive(Copy, Clone)]
pub struct update_task0<'a> {
    ve0: &'a Vec<(Locked<Position1>, LockedRef<Position3>, Ref<Position2>)>,
    rve0: &'a RwLock<HashSet<usize>>,
    ve1: &'a Vec<(Locked<Position1>, LockedRef<Position3>, Ref<Position2>)>,
...
impl<'a> EngineArch<(&'a dyn test,)> for update_task0<'a> {
    ...
    fn get_item(&self, mut index: usize) -> Option<(&'a dyn test,)> {
        if index < self.ve0.len() {
            return Some((&self.ve0[index].2,));
        };
        index -= self.ve0.len();
        if index < self.ve1.len() {
...
#[derive(Copy, Clone)]
pub struct long_task0<'a> {
    ve0: &'a Vec<(Locked<Position1>, LockedRef<Position3>, Ref<Position2>)>,
    rve0: &'a RwLock<HashSet<usize>>,
    ve1: &'a Vec<(Locked<Position1>, LockedRef<Position3>)>,
    rve1: &'a RwLock<HashSet<usize>>,
    len: usize,
}
impl<'a> long_task0<'a> {
...
impl<'a> EngineArch<(&'a Locked<Position1>,)> for long_task0<'a> {
    fn remove(&self, mut index: usize) {
        if index < self.ve0.len() {
            self.rve0.write().unwrap().insert(index);
            return;
        ...
        index -= self.ve1.len();
        eprintln!("Warning: index of out of {} is out of bounds", "long_task");
    }
    fn len(&self) -> usize {
        self.len
    }
