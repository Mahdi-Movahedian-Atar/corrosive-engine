...
#[derive(Copy, Clone)]
pub struct update_position0<'a> {
    ve0: &'a Vec<(Member<Position2D>, RendererMeta2D, Sprite2D)>,
    rve0: &'a RwLock<HashSet<usize>>,
    len: usize,
}
impl<'a> update_position0<'a> {
    pub fn new(
        ve0: &'a Vec<(Member<Position2D>, RendererMeta2D, Sprite2D)>,
        rve0: &'a RwLock<HashSet<usize>>,
    ) -> Self {
...
impl<'a> EngineArch<(&'a Member<Position2D>, &'a RendererMeta2D)> for update_position0<'a> {
    fn remove(&self, mut index: usize) {
        if index < self.ve0.len() {
            self.rve0.write().unwrap().insert(index);
            return;
    ...
    fn get_item(&self, mut index: usize) -> Option<(&'a Member<Position2D>, &'a RendererMeta2D)> {
        if index < self.ve0.len() {
