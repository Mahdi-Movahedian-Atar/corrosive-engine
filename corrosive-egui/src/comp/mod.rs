...
pub struct EguiObject{
    pub(crate) state: Option<(egui_winit::State,Box<dyn FnMut(&Context) + Send + Sync + 'static>)>,
...
impl Default for EguiObject{
    fn default() -> Self {
        EguiObject{state: None, renderer: None, textures: TexturesDelta::default(), input: true}
    }
}
impl EguiObject{
